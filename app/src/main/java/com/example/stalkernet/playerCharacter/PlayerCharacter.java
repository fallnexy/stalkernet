...
import com.example.stalkernet.MainActivity;
...
import static com.example.stalkernet.Discharge.DISCHARGE;
import static com.example.stalkernet.MainActivity.INTENT_MAIN;
import static com.example.stalkernet.MainActivity.INTENT_MAIN_MINE;
import static com.example.stalkernet.StatsService.CHANNEL_ID;
import static com.example.stalkernet.StatsService.INTENT_SERVICE;
import static com.example.stalkernet.StatsService.INTENT_SERVICE_SOUND;
...
public class PlayerCharacter {
    ...
    public static final String PROTECTIONS_AVAILABLE_KEY = "protections_available";
    ...
    * */
    public double getContaminationUnit(String type, int row){
        if (contaminationMap.get(type) == null){
            setDefaultContaminationMap();
        }
    ...
    * */
    ...
    /*
    * выдает строку матрицы переведенную в инт
    * */
    public int[] getIntContamination(boolean currentValues){
        int[] array = new int[3];
        for (int i = 0; i < 3; i++){
            array[i] = (int) contamination[currentValues ? 0 : 1][i];
        }
        return array;
    }
    /*
    ...
    * */
    ...
    /*
    * выдает масси totalProtection для rad bio psy
    * */
    public double[] getStringTotalProtections(){
        double[] allTotalProtections = new double[3];
        allTotalProtections[0] = getTotalProtection(radProtection[1]);
        allTotalProtections[1] = getTotalProtection(bioProtection[1]);
        allTotalProtections[2] = getTotalProtection(psyProtection[1]);
        return allTotalProtections;
    }
    /*
    ...
    * */
    ...
    protected void resolveMine(double chance, String protection){
        //Log.d(LOG_CHE, "available before = " + mineAvailable);
        ...
        if (isAppInForeground(context)) {
            ...
            if (mineAvailable) {
                ...
                if (randomNum <  newChance){
                    handlerMine.removeCallbacksAndMessages(null);
                    mineAvailable = false;
                    handlerMine.postDelayed(() -> setMineAvailable(true), MINE_COUNT_DOWN);
                    sendIntent(INTENT_MAIN,INTENT_MAIN_MINE, protection);
                    sendIntent(INTENT_SERVICE, INTENT_SERVICE_SOUND, MINE_ACTIVATION);
                }
            }
        } else {
            ...
            if(Build.VERSION.SDK_INT < 31){
                ...
                pendingIntent = PendingIntent.getActivity(getContext(), 0, intent, PendingIntent.FLAG_MUTABLE);
            }
            NotificationCompat.Builder builder = new NotificationCompat.Builder(getContext(), CHANNEL_ID)
                    .setSmallIcon(R.drawable.profile)
                    .setContentTitle("МИННОЕ ПОЛЕ")
                    .setContentText("Откройте приложение, чтобы иметь возможномть деактивировать мину")
            ...
            if (mineAvailable) {
                ...
                if (randomNum <  newChance){
                    ...
                    handlerMine = new Handler();
                    handlerMine.postDelayed(() -> setMineAvailable(true), MINE_COUNT_DOWN);
                    if (protection.equals("true")){
                        sendIntent(INTENT_SERVICE, INTENT_SERVICE_SOUND, MINE_DEACTIVATION);
                    } else {
                        increaseHealthPercent(MINE_DAMAGE_PERCENT);
                        sendIntent(INTENT_SERVICE, INTENT_SERVICE_SOUND, MINE_EXPLOSION);
                    }
                }
            }
        }
        //Log.d(LOG_CHE, "available after = " + mineAvailable);
    }
    private void sendIntent(String action, String name, String message){
    ...
    * */
    public void purification(){
        increaseContaminationUnit(RAD, -0.1);
